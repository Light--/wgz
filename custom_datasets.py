...
class datasetGen(Dataset):
    def __init__(self):
        f = open("imgs_NamesLbls.json") #img1 img2 img1_lbl img2_lbl
        train = json.load(f)
...
class siameseDataset(Dataset):
    def __init__(self):
        f = open("imgs_pairsGen.json") #img1 img2 img1_lbl img2_lbl
        train = json.load(f)
...
class tripletDataset(Dataset):
    def __init__(self):
        ...
        self.num = len(self.img_path)
        self.indices_to_labels = {label: np.where(np.array(self.imgLbl_path) == label)[0] for label in self.label_set}
