...
def pairsList():
    ...
    with open('imgs_NamesLbls.json') as f:
...
def tripList():
    out = {'img1':[],'img2':[],'img3':[]}
    out_path = 'JSONFiles/{}.json'.format('imgs_TripGen')
    ...
    for i in range(0,n-1):
        for j in range(0,n):
            anchor = imgs_names['img'][i]
            anchor_lbl = imgs_names['label'][i]
            img1 = imgs_names['img'][j]
            img1_lbl = imgs_names['label'][j]
